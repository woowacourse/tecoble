...
>
> 하지만 필자는 **2. 컬렉션의 불변성을 보장**에서는 불변이라고 확신하지 말라는 메시지를 전하고 싶다.
...
이렇게 수정하면 멤버변수에 저장되는 주소값을 재할당되기 때문에 영향을 받지 않는다.
...
필자가 이렇게 긴 설명을 한 이유는 앞서 말했듯이 **불변성을 보장**이라고 했지만 시행착오없이 확신하지 말하는 메시지를 전하기 위함이였다.
