...
- **테스트가 성공**해야 문서 작성됨
  - 테스트가 성공하지 않으면 문서가 작성되지 않는다. 이는 문서로 만들어 진 API의 신뢰도를 높이고 더불어 테스트 코드의 검증을 한 번 더 할 수 있다.
- 실제 코드에 **추가되는 코드가 없음**
  - 프로덕션 코드와 분리되어있기 때문에 배포 과정에 불필요한 파일 적재가 이뤄지지 않는다. Swagger처럼 문서화 관련 어노테이션이 범벅된 코드에 눈살 찌푸릴 일도 없다.
...
다음으로 findAll과 findById이다. (중복되는 설명은 생략하겠다.😂😂)
...
`src/main/docs/asciidoc`와 같이 디렉토리를 만들고 `*.adoc`파일을 작성해준다. ([Asciidoctor User Manual](https://asciidoctor.org/docs/user-manual/#introduction-to-asciidoctor) 참고)
...
IntelliJ를 사용한다면 AsciiDoc plugin을 설치해보자. 미리 보기가 가능해진다. (사용하는 것을 추천한다.👍)

![image](https://user-images.githubusercontent.com/45934117/90552659-23e5d980-e1ce-11ea-9e5c-1eea03b10ce0.png)

코드 작성을 완료했다면 다시 한 번 build한다. 그러면 `build/asciidoc/html5`와 `src/main/resources/static/docs`에 `html` 파일이 생길 것이다.
