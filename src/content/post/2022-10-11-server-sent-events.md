...
저희 팀은 처음에는 구현이 간단한 short polling 방식을 사용했으나, 사용자에게 높은 실시간성을 제공하기 위해 0.1초마다 서버에 요청을 보내다보니 서버에도 부담이 크고, 변경 사항이 없는데도 DOM을 계속 교체해야 하는 문제가 있었기 때문에 SSE 방식으로 변경하기로 결정했습니다.
