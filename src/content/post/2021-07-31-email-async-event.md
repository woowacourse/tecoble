...
서비스를 운영하는 데 Email 전송 기능을 도입하는 경우가 많다. 이메일 기능에 비동기와 이벤트 발행방식을 도입하게 된 이유와 방법에 관해서 설명해 보고자 한다.
...
사용하지 않고 Gmail 개인 계정을 사용하고 진행하였다. (보안 수준이 낮은 앱 허용 진행하여야 한다) 이 부분을 다루고 있는 블로그가 많으므로 코드를 남겨두고 별도의 설명은 생략한다.
...
> email, password를 github 저장소에 올리기 부담될 때에는 __private-submodule__을 사용해 보기를 추천한다.
...
public class AsyncConfiguration implements AsyncConfigurer {
    ...
    private static Loggerlogger=LoggerFactory.getLogger(AsyncConfiguration.class);
...
`@EnableAsync` 옵션만 주어도, 비동기 처리를 사용할 수 있다. 이를 Customizing 하기 위해서 위처럼, AsyncConfigurer를 구현한 설정 파일을 만들어 줄 수도 있다. 별도 설정을 하지 않은 경우, CorePoolSize 는 1, MaxPoolSize, QueueCapacity는 `Integer.MAX_VALUE` 로 정의되어 있고, warning 레벨의 log를 출력하도록 설정이 되어 있다.

### 기존 코드에 적용

위에는 비동기 처리를 위한 쓰레드를 별도로 생성해주는 설정 파일이고, 특정 메소드를 비동기 처리해주기 위해서 아래와 방식으로 메소드 위에 `@Async` 어노테이션을 붙여주면 된다. 별도로 명시해주면, 특정 쓰레드를 지정해줄 수도 있다. (클래스에 어노테이션을 붙여줄 시 전체 메소드에 적용)
...
우리 서비스는 Job의 `예약`, `취소`, `시작`, `종료`의 모든 경우에 대해서 메일을 보내주는 기능이 있다. 만약, 메일 뿐만 아니라 애플리케이션의 알람을 보내주는 로직이 추가된다고 가정해보자. 각각의 메소드에 일일이 추가되는 로직을 추가해줘야 하고, 도메인 로직에 집중하지 못하는 코드가 된다. 이벤트의 방법을 통해, JobService와 MailService의 의존성을 끊어줄 수 있고 확장 가능한 구조가 될 수 있다.
...
`EventListener`는 어노테이션을 통해서 구현이 가능한데, `TransactionalEventListener`를 활용하면, `TransactionaPhase`에 따른 EventListen 시점을 선택할 수 있다. 우리 서비스의 경우, AFTER_COMMIT(DB에 반영된 후)에 Event를 듣도록 설정이 되어 있다. JobEvent의 상속구조를 통해 여러 가지 상황(예약, 취소, 시작, 종료)을 하나의 메소드로 처리할 수 있다.
...
이 부분에 대해 더 공부해보고 싶은 분은 이 [블로그](https://supawer0728.github.io/2018/03/24/spring-event)를 추천하다. 도메인에만 더욱더 집중할 수 있는 그런 코드가 될 수 있을 것으로 생각한다.
