...
하지만 reservation 테이블을 조회하니, 락킹하지 않은 코드와 차이점을 발견할 수 있었다.
...
비관적 락은 싱글 DB 환경에만 적용 가능하다. 분산 DB 환경에서는 동시성 제어의 효력을 잃는다.
...
우리의 경우 싱글 DB 환경이고, 몇만 건의 동시 요청이 예상되는 상황은 아니므로 위의 단점들이 치명적이진 않다고 판단했다. 따라서 간단하게 적용할 수 있고, 데이터의 정합성이 유지되는 방법인 비관적 락을 통한 해결책을 채택하였다.
