...
위와 같이 `throttle`이라는 함수를 만들 때, 변수 `timerId`는 `let`이기 때문에 재할당이 가능합니다. 하지만 위와 같이 여러 곳에서 `throttle` 함수를 호출한다면, 쓰로틀링 동작은 우리가 기대했던 것과 같을까요? 여러 곳에서 호출되는 `throttle` 함수가 같은 `timerId`를 공유하고 있기 때문에, throttle을 호출할 때마다 다른 용도의 쓰로틀링에 사용되었을지 모를 `timerId` 값이 참조될 것입니다. 여러 곳에서 사용되는 `throttle`함수 각자가 자신만의 `timerId`를 갖고, 다른 `timerId`는 참조하지 못하도록 해주면 어떨까요? 이때 클로져가 역할을 할 수 있습니다.
