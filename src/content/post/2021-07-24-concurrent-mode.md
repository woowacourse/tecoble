...
우선 여러 작업을 동시에 할 수 있다면 어떤 점이 좋은지는 잠깐 미뤄두자.
필자는 개인적으로 두 눈으로 본 것 아니면 의심하는 경향이 있다.
리액트가 `정말 한 번에 하나의 작업만 처리하는지`부터 확인해보자.
...
위의 예제에서 아무 값이나 입력해보자. 예제에서는 사용자의 입력이 들어올 때마다 5,000개의 DOM element가 생성된다. 정상적으로 값을 입력하지 못할 정도로 버벅댈 것이다. 실제로는 5000개의 DOM element를 생성할 일은 거의 없지만, 렌더링 도중에 다른 모든 작업을 차단하는 모습을 직접 눈으로 보는 데에는 이만한 예제가 없다.
...
예시로 게임에 빠져 살던 시절 필자는 컴퓨터를 앞에 두고 밥을 같이 먹은 적이 종종 있었다. 게임에 중요한 상황이 있으면 마우스와 키보드를 잡았다가 게임에 신경 쓰지 않아도 되는 타이밍에 빠르게 밥과 김치를 입에 넣곤 했다.
손이 2개밖에 안 되어 실제로 식사와 게임을 `동시에` 하진 못했지만 작업 간의 전환을 빠르게 하여 동시에 수행하는 듯 보였다.
...
- 디바운스
  ...
  하지만 이 방법은 기기의 성능과는 관계없이 무조건 `일정 시간`을 기다려야 한다는 단점이 있다.
  슈퍼컴퓨터든 10년 된 컴퓨터든 똑같이 마지막 입력 이후에 0.5초를 기다려야 한다면 슈퍼 컴퓨터 입장에서는
  안타까울 수밖에 없다. 또한 사용자 입력 중에 무거운 작업의 처리가 이뤄지지 않는다는 점도 아쉬울 수 있다.
...
특히 예제를 보면 전체 로딩 화면이 아주 짧게 나오는 것을 확인할 수 있는데 사용자에게 피로감을 줄 수 있는 불필요한 로딩은 최대한 줄이는 것이 좋다.
...
> 준비 중인 UI의 `렌더링 단계`가 `특정 조건`에 부합하면 실제 DOM에 반영한다.

현 UI 상태를 유지한다는 것은 크게 어렵지 않으리라 생각한다.
...
이 부분은 [리액트 공식 문서](https://ko.reactjs.org/docs/concurrent-mode-patterns.html)에서도 모호하게 나와 있어 정확하게 알기는 어려웠다.
하지만 concurrent mode의 원리 중 하나인 `현재 UI를 유지한다`에 암시된 의미와 필자가 실험해본 결과를 조합해보았을 때,
...
Skeleton은 등장하지 않고 바로 다른 크루에 대한 정보가 렌더링 되는 Complete 상태로 넘어간다.
...
지금까지는 concurrent mode가 동작하는 원리를 알아보았다. 이제 concurrent mode를 실제 코드에 적용하기 위해서는 어떻게 해야 할까?
...
### 3. state 반영 우선순위를 정하기

우리는 사용자가 input을 입력할 때 버벅거림을 줄이기 위해서 input state의 우선순위를 높게 줄 것이고
5000개의 렌더링을 위한 array state는 우선순위를 낮게 주어 UI 업데이트가 지연될 수 있도록 할 것이다.
...
이렇게 해주는 이유는 사용자의 입력이 5,000개의 DOM element를 렌더링하는 것보다 우선시 되어야 사용자가 입력할 때의 버벅임을 줄일 수 있기 때문이다.
...
미리 공부하는 것이 나중에 큰 도움이 되리라 생각한다.

오늘 주제에 관해 공부하면서 사용자 경험의 관점에서 많은 생각을 하게 되었다. 많은 로딩이 사용자 경험에 좋지 않다는 점을 알게 되었고
